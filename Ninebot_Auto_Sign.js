...
const KEY_AUTOREPAIR = "ninebot.autoRepair"; // 自动补签开关
...
!(async () => {
  ...
  try {
    ...
    // 5) 自动补签（自动修复缺失签到日）
    if (cfg.autoRepair) {
      // 说明：自动补签使用 repair 接口，谨慎使用（会消耗补签卡）
      try {
        // 先查询状态看是否补签必要（示例逻辑：若连续天数为0且有补签卡，则尝试一次）
        if (st && st.code === 0) {
