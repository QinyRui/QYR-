...
function readPS(k){ try{ if(HAS_PERSIST) return $persistentStore.read(k); }catch{} return null; }
function writePS(v,k){ try{ if(HAS_PERSIST) return $persistentStore.write(v,k); }catch{} return false; }
...
function renderProgressBar(cur,total,style=0,len=20){
  try{ style=Number(style)||0; const [F,E]=PROGRESS_STYLES[style]||PROGRESS_STYLES[0]; const f=Math.round((cur/total)*len); return F.repeat(f)+E.repeat(Math.max(0,len-f)); }catch{return "██████------"; }
}
...
const progressStyle=argProgressStyle!==null ? argProgressStyle : Number(readPS(KEY_PROGRESS)||0);
  ...
  try{
    ...
    if(currentSignStatus===0||currentSignStatus==null){
      ...
      if(signResp?.code===0||signResp?.code===1){
        ...
        try{ const newStatus=await httpGet(`${END.status}?t=${Date.now()}`,headers); if(newStatus?.data?.consecutiveDays){ statusData=newStatus.data; consecutiveDays=statusData.consecutiveDays; currentSignStatus=statusData.currentSignStatus; signCards=statusData.signCardsNum; } }catch{}
    ...
    /* N 币 */
    let balLine="";
    try{ const b=await httpGet(END.balance,headers); if(b?.code===0) balLine=`- 当前 N 币：${b.data?.balance??b.data?.coin??0}`; }catch{}

    /* 盲盒 */
    let blindInfo=[];
    try{ const box=await httpGet(END.blindBoxList,headers); const notOpened=box?.data?.notOpenedBoxes||[]; notOpened.forEach(b=>{ const target=Number(b.awardDays); const left=Number(b.leftDaysToOpen); blindInfo.push({target,left,opened:target-left}); }); }catch{}
