...
function renderProgressBar(current,total,styleIndex=0,length=20){ try{ styleIndex=Number(styleIndex)||0; if(styleIndex<0||styleIndex>PROGRESS_STYLES.length-1) styleIndex=0; const [FULL,EMPTY]=PROGRESS_STYLES[styleIndex]; const ratio = total>0?current/total:0; const filled=Math.round(ratio*length); const empty=Math.max(0,length-filled); return FULL.repeat(filled)+EMPTY.repeat(empty); }catch(e){ return "██████████----------"; } }
  ...
  try{
    ...
    if(currentSignStatus===0||currentSignStatus===undefined||currentSignStatus===null){
      ...
      if(signResp){
        ...
        if(signResp.code===0||signResp.code===1){ const nCoin=Number(signResp.data?.nCoin??signResp.data?.coin??0); const score=Number(signResp.data?.score??0); todayGainNcoin+=nCoin; todayGainExp+=score; signMsg=`✨ 今日签到：成功\n🎁 签到奖励：+${score} 经验、+${nCoin} N 币`; }
        else if(signResp.code===540004||(signResp.msg&&/已签到/.test(signResp.msg))){ signMsg=`✨ 今日签到：已签到（接口）`; }
    ...
    }else{ signMsg=`✨ 今日签到：已签到`; logInfo("检测到今日已签到，跳过签到接口"); }
    ...
    if(cfg.shareTaskUrl){
      try{
        ...
        let shareResp=null;
        try{ shareResp=await httpPost(cfg.shareTaskUrl,headers,JSON.stringify({page:1,size:20})); }catch(e){ logWarn("分享 POST 查询异常，尝试 GET：",String(e)); try{ shareResp=await httpGet(cfg.shareTaskUrl,headers); }catch(e2){ logWarn("分享 GET 也失败：",String(e2)); } }
        ...
        if(Array.isArray(listArr)&&listArr.length>0){
          ...
          const todayArr=listArr.filter(it=>{ try{ const t=Number(it?.occurrenceTime||it?.time||it?.ts||0); if(!t) return false; return toDateKeyFromSec(t)===today; }catch(e){return false;} });
          todayArr.forEach(it=>{ shareGain+=Number(it.count??it.score??0); });
          if(shareGain>0) shareTaskLine=`🎁 今日分享奖励：+${shareGain} 积分（流水）`; todayGainExp+=shareGain;
          ...
          const unfinished=(tasks||[]).filter(item=>{ const type=String(item?.type||item?.taskType||"").toLowerCase(); const completed=(item?.completed===0||item?.completed===false)?false:Boolean(item?.completed); return type.includes("share")&&!completed; });
          logInfo("匹配到未完成分享任务数：",unfinished.length);
          for(const t of unfinished){
            try{ const taskId=t.id||t.taskId||t.task_id; if(!taskId) continue; const claim=await httpPost(END.reward,headers,JSON.stringify({taskId})); logInfo("尝试领取任务返回：",claim); if(claim?.code===0){ shareGain+=Number(t.score||t.reward||0); logInfo("领取成功：",taskId); } }catch(e){ logWarn("自动领取单项异常：",String(e)); }
          }
    ...
    try{ const cr=await httpGet(END.creditInfo,headers); creditData=cr?.data||{}; const credit=Number(creditData.credit??0); const level=creditData.level??null; let need=0; if(creditData.credit_upgrade){ const m=String(creditData.credit_upgrade).match(/还需\s*([0-9]+)\s*/); if(m&&m[1]) need=Number(m[1]); }else if(creditData.credit_range && Array.isArray(creditData.credit_range)&&creditData.credit_range.length>=2){ need=creditData.credit_range[1]-credit; } upgradeLine=`- 当前经验：${credit}${level?`（LV.${level}）`:''}\n- 距离升级：${need} 经验`; logInfo("经验信息：",creditData); }catch(e){ logWarn("经验信息查询异常：",String(e)); }

    // 余额查询
    let balLine="";
    try{ const bal=await httpGet(END.balance,headers); if(bal?.code===0) balLine=`- 当前 N 币：${bal.data?.balance??bal.data?.coin??0}`; logInfo("余额查询：",bal); }catch(e){ logWarn("余额查询异常：",String(e)); }

    // 盲盒列表
    let blindInfo=[];
    try{ const box=await httpGet(END.blindBoxList,headers); const notOpened=box?.data?.notOpenedBoxes??[]; if(Array.isArray(notOpened)&&notOpened.length>0){ notOpened.forEach(b=>{ const target=Number(b.awardDays); const left=Number(b.leftDaysToOpen); const opened=Math.max(0,target-left); blindInfo.push({target,left,opened}); }); } logInfo("盲盒列表：",blindInfo); }catch(e){ logWarn("盲盒查询异常：",String(e)); }

    // 自动开启盲盒
    if(cfg.autoOpenBox && blindInfo.length>0){ for(const b of blindInfo){ try{ if(Number(b.left)===0 && Number(b.target)===7){ logInfo("检测到7天盲盒可开，尝试开箱..."); try{ const openR=await httpPost(END_OPEN.openSeven,headers,JSON.stringify({})); logInfo("开箱返回：",openR); if(openR?.code===0) notify(cfg.titlePrefix,"盲盒开启","7天盲盒已自动开启并领取奖励"); }catch(e){ logWarn("7天开箱异常：",String(e)); } } }catch(e){ logWarn("盲盒自动开启单项异常：",String(e)); } }

    // Progress bar render
    let progressLines=blindInfo.map(b=>`- ${renderProgressBar(b.opened,b.target,cfg.progressStyle)} (${b.opened}/${b.target})`);

    // Compose final notification
    let finalMsg=[signMsg,shareTaskLine,upgradeLine,balLine,...progressLines].filter(Boolean).join("\n");
    if(cfg.notify) notify(cfg.titlePrefix,"签到完成",finalMsg);

    logInfo("九号自动签到完成，通知已发送。");
  }catch(e){
    logErr("九号自动签到异常：",e);
    if(cfg.notifyFail) notify(cfg.titlePrefix,"签到异常",String(e));
  }finally{
    logInfo("九号自动签到结束");
    $done();
  }
