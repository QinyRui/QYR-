/***********************************************
 Ninebot_Sign_Single_v2.6 流线增强版（最终修复）
 功能：抓包写入、自动签到、分享任务重放/领取、盲盒开箱、
       经验/N币查询、通知美化
***********************************************/
...
/* Logging controlled by plugin UI */
function logInfo(...a){ console.log(`[${nowStr()}] INFO`,...a); }
function logWarn(...a){ console.warn(`[${nowStr()}] WARN`,...a); }
function logErr(...a){ console.error(`[${nowStr()}] ERROR`,...a); }
...
if(IS_REQUEST && CAPTURE_PATTERNS.some(p=>($request?.url||"").includes(p))){
  try{
    ...
    if(changed) notify("九号智能电动车","抓包成功 ✓","Authorization / DeviceId / UA / shareTaskUrl 已写入");
...
const cfg = {
  ...
  progressStyle: Number(readPS(KEY_PROGRESS)||7),
  logLevel: readPS(KEY_LOGLEVEL)||"INFO"
};
if(!cfg.Authorization||!cfg.DeviceId){ notify(cfg.titlePrefix,"未配置 Token","请先抓包写入"); $done(); }

function makeHeaders(){ return {"Authorization":cfg.Authorization,"Content-Type":"application/json;charset=UTF-8","device_id":cfg.DeviceId,"User-Agent":cfg.userAgent||"Mozilla/5.0 (iPhone) Segway v6","platform":"h5","Origin":"https://h5-bj.ninebot.com","language":"zh"}; }
...
(async()=>{
  try{
    ...
    let s=statusResp?.data||{};
    let consecutiveDays = s.consecutiveDays??s.continuousDays??0;
    let signCards = s.signCardsNum??s.remedyCard??0;
    let currentSignStatus = s.currentSignStatus??null;
    ...
    if(currentSignStatus===0||currentSignStatus==null){
      ...
      if(signResp?.code===0||signResp?.code===1){
        ...
        try{
          ...
          s=newStatus?.data||{};
          consecutiveDays=s.consecutiveDays??consecutiveDays;
          currentSignStatus=s.currentSignStatus??currentSignStatus;
          signCards=s.signCardsNum??signCards;
    ...
    if(cfg.shareTaskUrl){
      let share=null;
      try{ share=await httpPost(cfg.shareTaskUrl,headers,JSON.stringify({page:1,size:20})); }catch(e){ try{ share=await httpGet(cfg.shareTaskUrl,headers); }catch(e2){} }
    ...
    if(todayGainExp||todayGainNcoin){
      lines.push(""); lines.push(`🎯 今日获得：经验 ${todayGainExp} / N币 ${todayGainNcoin}`);
    }

    if(cfg.notify) notify(`${cfg.titlePrefix} · 今日签到结果`,"",lines.join("\n"));
  }catch(e){ notify(cfg.titlePrefix,"脚本异常",String(e)); }finally{ $done(); }
})();