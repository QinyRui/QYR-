/***********************************************
 Ninebot_Sign_Single_v2.6_final.js —— ES5 完全兼容版
 2025-11-30 19:00 完整整合版
...
if (typeof $argument === "undefined" || !$argument) {
  ...
  try { if (typeof $argument === "string") $argument = JSON.parse($argument); } catch (e) { if (typeof $argument !== "object") $argument = {}; }
}
...
function renderProgressBar(current,total,styleIndex,length){
  try{
    ...
    var ratio=(total>0?current/total:0),filled=Math.round(ratio*length);
...
var cfg={
  ...
  notify:(readPS(KEY_NOTIFY)==="false"?false:true),       // 只控制抓包通知
...
if(isCaptureRequest){
  try{
    ...
    if(changed&&cfg.notify){notify(cfg.titlePrefix,"抓包成功 ✓","Authorization / DeviceId / User-Agent / shareTaskUrl（若捕获）已写入 BoxJS");logInfo("抓包写入成功");}
    else logInfo("抓包数据无变化（已写入 BoxJS 的数据与当前抓到的相同）");
  ...
  try{
    ...
    try{
      ...
      if(creditInfo&&creditInfo.data){
        var data=creditInfo.data,credit=safeNum(data.credit,0),level=data.level!==undefined?data.level:null,need=0;
    ...
    var balLine="";
    try{
      var bal=await httpGet(END.balance,headers).catch