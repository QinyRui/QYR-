/***********************************************
 Ninebot_Sign_Single_v2.6.js  （版本 D · 修正版）
 2025-12-01 01:30 更新版（积分/N币统计增强 + 简化盲盒显示）
 功能：抓包写入、自动签到、分享任务领取、盲盒显示、经验/N币查询、通知美化
***********************************************/
...
if (isCaptureRequest) {
  try {
    ...
    if (changed) {
      writePS(String(Date.now()), "ninebot.lastCaptureAt");
      notify("九号智能电动车","抓包成功 ✓","Authorization / DeviceId / User-Agent / shareTaskUrl 已写入 BoxJS");
      logInfo("抓包写入成功");
  ...
  return new Promise((resolve,reject)=>{
    ...
    const once=()=>{
      ...
      const cb=(err,resp,data)=>{
        if(err){ const msg=String(err && (err.error||err.message||err)); if(attempts<MAX_RETRY&&/(Socket closed|ECONNRESET|network|timed out|timeout|failed)/i.test(msg)){ setTimeout(once,RETRY_DELAY); return; } else { reject(err); return; } }
...
(async()=>{
  try{
    ...
    // 1) 查询签到状态
    logInfo("查询签到状态...");
    const statusResp = await httpGet(`${END.status}?t=${Date.now()}`, headers).catch(e=>{logWarn("状态请求异常：",e); return {};});
      ...
      try{
        ...
        if(signResp.code===0 || signResp.code===1 || signResp.success===true){
          ...
          if(Array.isArray(rewardList)){
            for(const r of rewardList){ const val=Number(r.rewardValue??0); const type=Number(r.rewardType??0); if(type===1) newExp+=val; else newCoin+=val; }
          }
          const nCoin=Number(signResp.data?.nCoin??signResp.data?.coin??0);
          const score=Number(signResp.data?.score??signResp.data?.credit??0);
          todayGainExp += (score+newExp); todayGainNcoin += (nCoin+newCoin);
          ...
          signMsg = `✨ 今日签到：已签到（接口）`;
        }else{ const rawMsg=signResp.msg??signResp.message??JSON.stringify(signResp); if(cfg.notifyFail) signMsg=`❌ 签到失败：${rawMsg}`; }
      }catch(e){ logWarn("签到请求异常：",e); if(cfg.notifyFail) signMsg=`❌ 签到请求异常：${e}`; }
    ...
    try{
      ...
      for(const it of nCoinList){
        const k=toDateKeyFromAny(it.create_time??it.createDate??it.createTime??it.create_date);
        if(k===today) todayGainNcoin+=Number(it.amount??it.coin??it.value??0);
      }
      logInfo("今日积分/ N币统计完成：",todayGainExp,todayGainNcoin);
    }catch(e){ logWarn("积分/N币统计异常：",e); }
    ...
    try{
      ...
      if(Array.isArray(notOpened) && notOpened.length>0){
        notOpened.forEach(b=>{ const target=Number(b.awardDays??b.totalDays??b.daysRequired??0); const left=Number(b.leftDaysToOpen??b.remaining??0); const opened=Math.max(0,target-left); blindInfo.push({target,opened}); });
      }
        ...
        try{
          if(b.target===7 && b.opened===7){
            ...
            try{
              ...
              logInfo("开箱返回：",openR);
              if(openR?.code===0){ notify(cfg.titlePrefix,"盲盒开启","7天盲盒已自动开启并领取奖励"); logInfo("7天盲盒开启成功"); }
              else logWarn("7天盲盒开箱未成功：",openR);
            }catch(e){ logWarn("7天开箱异常：",e); }
          }
        }catch(e){ logWarn("盲盒处理异常：",e); }
      }
    }
