/***********************************************
 ...
 2025-11-29
 功能：抓包写入、自动签到、分享任务领取、盲盒开箱、经验/N币查询
      支持 7 种盲盒进度条样式
***********************************************/
...
const KEY_AUTH="ninebot.authorization",KEY_DEV="ninebot.deviceId",KEY_UA="ninebot.userAgent",
      KEY_NOTIFY="ninebot.notify",KEY_AUTOBOX="ninebot.autoOpenBox",KEY_AUTOREPAIR="ninebot.autoRepair",
      KEY_NOTIFYFAIL="ninebot.notifyFail",KEY_TITLE="ninebot.titlePrefix",KEY_SHARE="ninebot.shareTaskUrl",
      KEY_PROGRESS="ninebot.progressStyle";
...
const cfg={
  ...
  progressStyle:argProgressStyle!==null?argProgressStyle:boxProgressStyle
};

if(!cfg.Authorization||!cfg.DeviceId){ notify(cfg.titlePrefix,"未配置 Token","请先抓包并执行签到/分享动作"); $done(); }
...
(async()=>{
  try{
    ...
    if(cfg.shareTaskUrl){
      ...
      catch{try{share=await httpGet(cfg.shareTaskUrl,headers);}catch{}}
    ...
    try{
      ...
      let need=0;
      if(d.credit_upgrade){
        const m=String(d.credit_upgrade).match(/([0-9]+)/); if(m) need=Number(m[1]);
      }
    ...
    /* 自动开7天盲盒 */
    if(cfg.autoOpenBox){
      for(const b of blindInfo){ if(b.target===7&&b.left===0){ try{ const r=await httpPost(END_OPEN.openSeven,headers,JSON.stringify({})); if(r?.code===0) notify(cfg.titlePrefix,"盲盒开启","7天盲盒奖励已领取");}catch{} } }
    }
