...
(async ()=>{
  try{
    ...
    /* 通知只显示签到结果 */
    if(cfg.notify) notify(cfg.titlePrefix+" · 今日签到结果","",signMsg);

    /* 控制台日志输出（保留全量） */
    logInfo("当前配置：", cfg);
    logInfo("查询签到状态...");
    logInfo("签到状态：", statusData);
    if(currentSignStatus===0||currentSignStatus==null) logInfo("今日未签到，执行签到接口...");
    else logInfo("今日已签到，跳过签到接口调用");
    if(cfg.shareTaskUrl && shareGain===0) logInfo("分享任务接口返回无列表或格式不支持：", JSON.stringify(share||{}));
    logInfo("经验信息：", upgradeLine);
    logInfo("余额查询：", balLine);
