/***********************************************
 Ninebot_Sign_Single_v2.6.js  （版本 D · 最终闭合版）
 更新日期：2025-11-30 08:50
 功能：
  ...
  try{
    ...
    if(cfg.shareTaskUrl){
      try{
        ...
        catch(e){ logWarn("分享 POST 异常，尝试 GET：",String(e)); try{ shareResp = await httpGet(cfg.shareTaskUrl,headers); }catch(e2){ logWarn("分享 GET 也失败：",String(e2)); } }
