...
const END={
  ...
  creditList:"https://api5-h5-app-bj.ninebot.com/web/credit/list", // 积分流水
  shareTask:"https://snssdk.ninebot.com/service/2/app_log/?aid=10000004" // 分享任务
};
  ...
  try{
    ...
    try{
      ...
      if(shareResp?.code===0 && Array.isArray(shareResp.data?.list)){
        ...
        shareTaskLine=`\n📌 今日分享任务：${todayShares.length>0?'已完成':'未完成'}，获得 ${shareGain} 积分`;
        todayGainExp+=shareGain; // 累加今日总积分
      }
    ...
    try{
      ...
      if(creditInfo?.code===1 && creditInfo.data){
        ...
        upgradeLine=`\n📈 当前经验：${credit}${level?`（LV.${level}）`:''}，距离升级还需 ${need}`;
      }
