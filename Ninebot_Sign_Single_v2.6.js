/***********************************************
 Ninebot_Sign_Single_v2.6.js  （版本 D · 最终整合）
 2025-11-29
 功能：抓包写入、自动签到、分享任务重放/领取、盲盒开箱、经验/N币查询、通知美化
 说明：主体脚本保留日志输出，BoxJS 控制盲盒样式、日志等级、通知标题
***********************************************/
...
const KEY_DEBUGLEVEL="ninebot.debugLevel";
    ...
    const once=()=>{
      ...
      const cb=(err,resp,data)=>{
        if(err){
          ...
          const retry=/(Socket closed|ECONNRESET|network|timed out|timeout|failed)/i.test(msg);
          if(attempts<MAX_RETRY && retry) return setTimeout(once,RETRY_DELAY);
          return reject(err);
        }
...
function renderProgressBar(cur,total,style=0,len=20){
  try{
    ...
    const f=Math.round((total>0?cur/total:0)*len);
...
const cfg={
  ...
  debugLevel:Number(readPS(KEY_DEBUGLEVEL)||1)
};
...
(async()=>{
  try{
    ...
    if(currentSignStatus===0||currentSignStatus==null){
      ...
      if(signResp?.code===0||signResp?.code===1){
        ...
        if(newStatus?.data?.consecutiveDays){ statusData=newStatus.data; consecutiveDays=statusData.consecutiveDays; signCards=statusData.signCardsNum; currentSignStatus=statusData.currentSignStatus; }
    ...
    /* 5) N币 */
    let balLine="";
    try{ const b=await httpGet(END.balance,headers); if(b?.code===0) balLine=`- 当前 N币：${b.data?.balance||b.data?.coin||0}`; }catch(e){}

    /* 6) 盲盒 */
    let blindInfo=[];
    try{ const box=await httpGet(END.blindBoxList,headers); const notOpened=box?.data?.notOpenedBoxes||[]; notOpened.forEach(b=>{ blindInfo.push({target:Number(b.awardDays),left:Number(b.leftDaysToOpen),opened:Number(b.awardDays)-Number(b.leftDaysToOpen)}); }); }catch(e){}

    /* 自动开7天盲盒 */
    if(cfg.autoOpenBox){ for(const b of blindInfo){ if(b.target===7&&b.left===0){ try{ const r=await httpPost(END_OPEN.openSeven,headers,JSON.stringify({})); if(r?.code===0) notify(cfg.titlePrefix,"盲盒开启","7天盲盒奖励已领取"); }catch(e){} } } }
  ...
  }catch(e){ logErr("脚本异常",e); notify(cfg.titlePrefix,"脚本异常",String(e)); }
