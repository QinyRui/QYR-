...
if (isReq) {
  try {
    ...
    if (changed) {
      ...
      log("[Ninebot] 抓包写入成功:", {auth, dev, ua});
    }
  } catch (e) {
    log("[Ninebot] 抓包写入异常：", e.message || safeStr(e));
  }
  $done({});
}
    ...
    try {
      $httpClient.post({ url, headers, body }, (err, resp, data) => {
        if (err) reject(err);
          ...
          catch (e) { log("[Ninebot] HTTP 解析异常：", e.message || safeStr(e)); resolve({ raw: data }); }
        }
      });
    } catch (e) { log("[Ninebot] httpPost 异常：", e.message || safeStr(e)); }
  });
}
...
function httpGet({ url, headers }) {
  return new Promise((resolve, reject) => {
    ...
    } catch (e) { log("[Ninebot] httpGet 异常：", e.message || safeStr(e)); }
  });
}
...
!(async () => {
  ...
  try {
        ...
        if (ready.length > 0) {
          ...
          for (const b of ready) {
            try {
              ...
              log("盲盒领取异常：", e.message || safeStr(e)); 
              notifyBody += `\n❌ ${b.awardDays}天盲盒领取异常`;
            }
          }
        }
    ...
    if (cfg.autoRepair) {
      ...
      } catch (e) { log("自动补签异常：", e.message || safeStr(e)); }
    }
    ...
    log("主流程异常：", e.message || safeStr(e));
    notify(cfg.titlePrefix,"脚本异常",e.message || safeStr(e));
  }
