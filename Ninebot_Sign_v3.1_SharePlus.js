...
💬 功能：
  ...
  - BoxJS 昵称 + 全量通知
📦 版本：v3.1 Share+
💻 适用平台：Loon / Surge / Quantumult X / BoxJS
*/
...
function httpClientGet(opts) {
  return new Promise((resolve, reject) => {
    $httpClient.get(opts, (err, resp, data) => err ? reject(err) : resolve({ resp, data }));
  });
}
function httpClientPost(opts) {
  return new Promise((resolve, reject) => {
    $httpClient.post(opts, (err, resp, data) => err ? reject(err) : resolve({ resp, data }));
  });
}
...
(async () => {
  ...
  if (!accounts) {
    notify("九号签到 v3.1", "❌ 未配置账号，请先抓取 Authorization & deviceId");
    return $done();
  }
